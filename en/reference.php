...
while ($row=mysqli_fetch_row($book_info))
{
	...
	echo ($row[1]);
	echo "</h1>";
	echo "<h3 style='margin-left:5%; text-align:left;'>Wikipedia {{cite book}} template</h3>";
	...
	echo " }}</ref>{{rp|}}";
	echo "</textarea>";
	echo "<button style='height:35px; width:50px; margin-left:20px; margin-top:11px; position: absolute;' onclick='citebook()'>Copy</button>";
}
$book_info=mysqli_query($db,"SELECT * FROM books WHERE hscode='$hscode'");
while ($row=mysqli_fetch_row($book_info))
{
	echo "<h3 style='margin-left:5%; text-align:left;'>Bibliographic Description and Recording Rules (China, GB/T 7714－2015)</h3>";
	...
	if($row[4]!=NULL){
		echo "．";}
	echo "</textarea>";
	echo "<button style='height:35px; width:50px; margin-left:20px; margin-top:3px; position: absolute;' onclick='chinacite()'>Copy</button>";
}
$book_info=mysqli_query($db,"SELECT * FROM books WHERE hscode='$hscode'");
while ($row=mysqli_fetch_row($book_info))
{
	echo "<h3 style='margin-left:5%; text-align:left;'>Vancouver Reference Style</h3>";
	...
	if($row[4]!=NULL){
		echo ".";}
	echo "</textarea>";
	echo "<button style='height:35px; width:50px; margin-left:20px; margin-top:3px; position: absolute;' onclick='vancouver()'>Copy</button>";
}
mysqli_close($db);
?>
		</div>
